...
const App: React.FC = () => {
  ...
  // 마지막 페이지에서 텍스트가 잘리지 않도록 높이를 최적화했습니다.
  const getImageHeight = () => {
    if (currentIndex === 4) return 'h-[16dvh] sm:h-[22%]'; // 이미지 높이를 더 줄여서 텍스트 공간 확보
      ...
      >
        ...
        <div className={`flex-1 flex flex-col px-6 min-h-0 overflow-hidden bg-white transition-all ${currentIndex === 4 ? 'pt-2 pb-0' : 'pt-5 pb-3'}`}>
          ...
          <div className={`${currentIndex === 0 ? 'mt-1' : ''} flex-shrink-0`}>
            {/* 제목: 마지막 페이지 여백 최소화 */}
            <h1 className={`${currentIndex === 4 ? 'text-[clamp(1.1rem,3.2vh,1.3rem)] mb-1 sm:mb-1' : 'text-[clamp(1.2rem,4vh,1.45rem)] mb-2 sm:mb-4'} font-[900] text-gray-900 leading-[1.3] whitespace-pre-wrap tracking-tighter break-keep`}>{currentCard.title}</h1>
          </div>

          <div className={`flex-1 flex flex-col no-scrollbar overflow-y-auto ${currentIndex >= 3 ? 'space-y-1 sm:space-y-1.5' : 'space-y-2 sm:space-y-2.5'}`}>
            {/* 소제목 */}
            ...
            {/* 본문: 마지막 페이지 글자 크기 미세 조정 */}
            {currentCard.body && <p className="text-gray-600 text-[clamp(0.85rem,2.15vh,1.05rem)] leading-[1.5] font-bold whitespace-pre-wrap break-keep tracking-tight">{currentCard.body}</p>}
          ...
          {/* 마지막 페이지 하단 액션: 버튼 크기 축소 및 간격 최적화 */}
          {isLastPage && (
            <div className="mt-2 mb-2 flex-shrink-0 flex flex-col items-center gap-0.5">
              <button 
                onClick={() => window.open(CARD_NEWS_CONFIG.links.donation, '_blank')} 
                className="w-full max-w-[260px] bg-emerald-500 text-white py-2 sm:py-2.5 rounded-xl font-black text-[13px] sm:text-[16px] flex flex-col items-center justify-center shadow-md active:scale-95 transition-transform"
              >
                <div className="flex items-center gap-1.5"><Heart className="w-3.5 h-3.5 fill-current" /><span>지금 정기 후원하기</span></div>
                <span className="text-[8px] sm:text-[10px] font-medium mt-0.5 opacity-90">발달장애인의 내일을 지원해주세요</span>
              </button>
